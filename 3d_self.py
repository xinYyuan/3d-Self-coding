...
#define network
    ...
    conv1=conv_3d(int_put,24,[24,3,3],padding='VALID',strides=[1,1,1,1,1],activation='prelu',weight_decay=0.05)
    ...
    conv2 = conv_3d(batch_norm, 12, [24, 3, 3], padding='VALID',strides=[1,1,1,1,1],activation='prelu',weight_decay=0.05)
    ...
    net=residual_block_concat(batch_norm, 2, 16,batch_norm=None,downsample_strides=1,weight_decay=0.05)
    ...
    ip1=fully_connected(flat,100,activation='prelu',)
    dro = dropout(ip1, 0.9)
    ip2=fully_connected(dro,20,activation='softmax',)
    network = regression(ip2, optimizer='Adagrad', loss='categorical_crossentropy', learning_rate=0.01)

    model = tflearn.DNN(network, tensorboard_verbose=0,tensorboard_dir="./tflearn_logs/")
    model.fit(x_train, y_train, n_epoch=200, shuffle=True, validation_set=(x_test, y_test),
                  show_metric=True, batch_size=32, run_id='3d_net')
...
def self(x_train, y_train, x_test, y_test):
    ...
    for i in range(10):
        model.fit(x_train, x_train, n_epoch=20, shuffle=True, show_metric=True,
              validation_set=(x_test, x_test),
              batch_size=32, run_id='3d_net_self')
    ...
    '''
    feature_model = tflearn.DNN(decon2)
    feature_model.load('my_model')
    x_feature = feature_model.predict([x_train[0, :, :, :,:]])
    save_hdf5(x_feature)
    print('asd')
    '''
